    ...
    for day in month_days:
        if day.month == month:
            calendar_days.append({
                ...
                "attended": day in attended_dates,
                "is_today": day == today,
                "is_future": day > today
            })
        else:
...
def mark_attendance(member_id):
    ...
    if current_user.role == 'trainer' and member not in current_user.assigned_members:
        flash('You do not have permission to mark attendance for this member.', 'error')
        return redirect(url_for('trainer_dashboard'))
    
    # Get current month and year from request args, or use today's date
    ...
    return render_template('mark_attendance.html', member=member, calendar_days=calendar_days,
                           calendar_title=calendar_title, prev_month=prev_month, next_month=next_month)
...
def toggle_attendance(member_id, date_str):
    ...
    if current_user.role == 'trainer' and member not in current_user.assigned_members:
        flash('You do not have permission to modify attendance for this member.', 'error')
        return redirect(url_for('trainer_dashboard'))
    ...
    if existing_record:
        ...
        flash(f'Attendance removed for {member.full_name} on {date_str}.', 'success')
    else:
